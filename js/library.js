...
function initialise(node, conference, userbw, name, userpin, registration_token) {
...
var class_transcribing_on = "participant_action fa fa-pencil-square-o fa-fw green";
...
function Participant(participant_data) {
    ...
    this.recording_element.addEventListener('click', this.recording.bind(this), false);
    ...
    this.transcribing_element.addEventListener('click', this.transcribing.bind(this), false);
    ...
    this.studiosound_element.addEventListener('click', this.studiosound.bind(this), false);
...
Participant.prototype.studiosound = function () {
    ...
    if (this.enable_media_buttons) {
        ...
        user.post_request("participants/" + this.uuid + "/studiosound", { "enabled" : this.studiosound_state});
    }
};
...
Participant.prototype.recording = function() {
    ...
    user.post_request("participants/" + this.uuid + "/calls/" + this.uuid + "/recording", recording_data);
...
Participant.prototype.transcribing = function() {
    ...
    user.post_request("participants/" + this.uuid + "/calls/" + this.uuid + "/transcribing", transcribing_data);
...
var user = {
    ...
    getrtmpaddress : function() {
        if (this.token) {
            ...
            if (response["status"] == 200) {
                ...
                if (dual_stream) {
                    var presentation_url = response["data"]["result"]["url"] + "-presentation";
                    console.log("presentation url: " + presentation_url);
                    var pres_address = document.getElementById("rtmpaddress_presentation");
    ...
    dial : function(event) {
        ...
        this.action_dial(protocol.value, localalias.value, localdisplayname.value, remotealias.value, role.value, prefer_ipv6.checked);
    },

    action_dial : function(protocol, source, source_display_name, destination, role, prefer_ipv6) {
    ...
    transformlayout: function (event) {
        ...
        if (tl_enable_overlay_text.value == "positional") {
            if (tl_positionaltext.value) {
                transforms["free_form_overlay_text"] = tl_positionaltext.value.split(",");
            } else {
                transforms["free_form_overlay_text"] = [];
            }
        }

        var bools = ["recording_indicator", "streaming_indicator", "transcribing_indicator", "enable_overlay_text", "enable_active_speaker_indication"];
...
var reg = {
   ...
   //Same info as previous section, but with an auto-confirm 
     incoming : function(event) {
             ...
             document.getElementById("worker").value = this.node;
             document.getElementById("pin").value = "";
             initialise(this.node, incoming_data["conference_alias"], undefined, "test room", undefined, incoming_data["token"]);
